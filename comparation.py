...
def comparative_dice(percent,name_model,fold_out,fold_in):
    ...
    #y_HR = [1,2,1,1] #np.asarray(dice_HR)
    #y_dist_paral = [2,3,2,2] #np.asarray(dice_dist_paral)
    #y_dist_seq = [3,4,3,3] #np.asarray(dice_dist_seq)
...
def comparative_perce_plot(out_file='HR',stage='test'):
    ...
    for fold_in in range(5):
            ...
            y_HR_4 = np.asarray(dice_HR_4)
            y_HR_5 = np.asarray(dice_HR_5)


            x =np.asarray(list(range(0,len(y_HR_1))))
            #print('fout{}_fin{}'.format(fold_out, fold_in))
            ax1.plot(x,y_HR_1,label = (('fout{}_fin{}').format(fold_out, fold_in)))
            ax1.set_xticks(x)
            ax1.legend()  # plot items
            ax1.grid(True)
            ax2.plot(x,y_HR_2,label = (('fout{}_fin{}').format(fold_out, fold_in)))
            ax2.set_xticks(x)
            ax2.legend()  # plot items
            ax2.grid(True)
            ax3.plot(x,y_HR_3,label = (('fout{}_fin{}').format(fold_out, fold_in)))
            ax3.set_xticks(x)
            ax3.legend()  # plot items
            ax3.grid(True)
            ax4.plot(x,y_HR_4,label = (('fout{}_fin{}').format(fold_out, fold_in)))
            ax4.set_xticks(x)
            ax4.legend()  # plot items
            ax4.grid(True)
            ax5.plot(x,y_HR_5,label = (('fout{}_fin{}').format(fold_out, fold_in)))
            ax5.set_xticks(x)
            ax5.legend()  # plot items
            ax5.grid(True)

            y_1_all.append(y_HR_1)
            y_2_all.append(y_HR_2)
            y_3_all.append(y_HR_3)
            y_4_all.append(y_HR_4)
            y_5_all.append(y_HR_5)
    ...
    #print('dice_all_HR_20',np.mean(dice_out_25),dice_out_25)
    #print('dice_all_HR_40',np.mean(dice_out_45),dice_out_45)
    #print('dice_all_HR_80',np.mean(dice_out_70),dice_out_70)
    plt.tight_layout()    
    plt.show()
...
def water_images_v1(path):
    ...
    for path_mask in test_file_names:
        ...
        #print(sum_percent)
    pvalue_1,pvalue_2,pvalue_3,pvalue_4,pvalue_5=comparative_perce_plot(out_file='HR',stage='test') #considering shuffle=False
...
def main():   
    ...
    arg('--percent', type=str, default='8,20,40,80', help='For example HR, paral, seq')
    ...
    for perc in percent:
        ...
        y_dist_paral_all=[]
        y_dist_seq_all=[]
        #print('0-hr',y_HR_all,'paral',y_dist_paral_all,'seq',y_dist_seq_all)   
        ...
        for i in (fold_out):
            ...
            for j in (fold_in):
                ...
                y_HR,y_dist_paral,y_dist_seq= comparative_dice(perc,name_model=args.name_model,fold_out=int(i),fold_in=int(j)) 
                #print('2-0-hr',y_HR,'paral',y_dist_paral,'seq',y_dist_seq)   


                comparative_plot(perc, y_HR, y_dist_paral, y_dist_seq,fold_out=int(i),fold_in=int(j),average='noaverage')
                
                y_HR_all.append(y_HR)
                #print('2-1-hr',y_HR_all,'paral',y_dist_paral_all,'seq',y_dist_seq_all)   

                y_dist_paral_all.append(y_dist_paral)
                #print('2-2-hr',y_HR_all,'paral',y_dist_paral_all,'seq',y_dist_seq_all)   

                y_dist_seq_all.append(y_dist_seq)
                #print('1-hr',y_HR,'paral',y_dist_paral,'seq',y_dist_seq)   #(y_HR[89],y_dist_paral[89])
